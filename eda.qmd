...
ctl_expanded <- ctl_train |>
 ...
 ver() |> 
 select(!where(is.ordered), -c(msisdn_dd, fct_srvy_dt, fecha_15dias)) |> 
 mutate(across(starts_with("fecha"), as_factor))
```
...
Utilizaremos Symmetrical Uncertainty (SU). SU es una métrica normalizada que cae
en el rango de 0 a 1, lo que facilita la comparación entre diferentes variables
que podrían tener diferentes unidades o escalas. Esto es especialmente útil si
se tienen variables que son fundamentalmente diferentes, como en este caso, en
términos de su naturaleza o escala de medición.
...
mi <- information_gain(
 formula      = diag ~ ., 
 data         = ctl_expanded, 
 type         = "symuncert", # Symmetrical Uncertainty (SU)
 discIntegers = FALSE) |> 
 as_tibble() |> 
 arrange(-importance)
toc()
...
mi |> 
 ...
 geom_point() +
 labs(title = "Symmetrical Uncertainty (SU)") +
 drako +
...
#| fig-height: 6
#| fig-width: 8
walk(lista_densidad[10], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_densidad[11], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_densidad[12], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_densidad[13], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_densidad[14], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_densidad[15], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_densidad[16], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_densidad[17], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_densidad[18], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_densidad[19], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_densidad[20], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_densidad[21], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_densidad[22], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_densidad[23], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_densidad[24], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_densidad[25], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_densidad[26], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_densidad[27], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_densidad[28], ~ print(.x))
```

Tras la construcción de gráficos de densidad con `geom_density_ridges_gradient`,
se observa que la mayoría de las métricas presentan distribuciones similares
entre las diferentes categorías de diagnóstico, especialmente después de la
normalización de los datos. Esto sugiere que estas métricas podrían no ser
particularmente discriminativas para diferenciar entre las categorías. Sin
embargo, hay excepciones notables, como las métricas `time_cl` y `time_lte`, que
muestran variaciones más significativas en sus distribuciones. En particular,
`time_cl` parece ser bimodal, lo que podría indicar la presencia de subgrupos
dentro de las categorías que podrían requerir una investigación más detallada.
Las métricas de modulación, por otro lado, tienden a seguir una distribución
cercana a la normal en todas las categorías, lo que podría simplificar cualquier
análisis estadístico posterior. Es importante señalar que, aunque las
visualizaciones de densidad ofrecen una visión general útil, algunas diferencias
sutiles entre categorías podrían no ser fácilmente apreciables y podrían
requerir un análisis más profundo para su completa comprensión.
...
#| fig-height: 6
#| fig-width: 8
walk(lista_boxplots[10], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_boxplots[11], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_boxplots[12], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_boxplots[13], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_boxplots[14], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_boxplots[15], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_boxplots[16], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_boxplots[17], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_boxplots[18], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_boxplots[19], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_boxplots[20], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_boxplots[21], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_boxplots[22], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_boxplots[23], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_boxplots[24], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_boxplots[25], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_boxplots[26], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_boxplots[27], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
walk(lista_boxplots[28], ~ print(.x))
```

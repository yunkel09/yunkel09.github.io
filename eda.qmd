...
**Si la última letra termina en UWXYZ entonces la celdas podría estar apagada**
Podríamos excluir con base a la tabla de Kenny aquellas celdas que no empiecen
con alguno de los prefijos que maneja Kenny.  Kenny nos compartirá la tabla.
Si aparece en cero en todas las modulaciones, podría ser que no tiene activado
los contadores.
- No hay modulación para 3G, solo para LTE
- Probar una receta filtrando todo los 3G y otra con todos los 3G y LTE.
...
walk(analisis_bivariado[10:16], ~ print(.x))
```
...
#| fig-height: 6
#| fig-width: 8
#| label: fig-modulation-qam
#| fig-cap: Índice de calidad general de la celda
walk(analisis_bivariado[17:18], ~ print(.x))
```
...
#| fig-cap: Índice de calidad general de la celda
walk(analisis_bivariado[19:20], ~ print(.x))
```
...
#| label: fig-time-lte
#| fig-cap: Índice de calidad general de la celda
walk(analisis_bivariado[21:22], ~ print(.x))
```
...
#| fig-cap: Índice de calidad general de la celda
walk(analisis_bivariado[23:24], ~ print(.x))
```
...
#| label: fig-rain
#| fig-cap: Índice de calidad general de la celda
walk(analisis_bivariado[25:26], ~ print(.x))
```
...
Al enfrentarnos a un nuevo conjunto de datos, especialmente uno con cientos o
miles de características, el proceso puede sentirse abrumador. Un excelente
primer paso es utilizar una métrica de utilidad de características para
clasificar la importancia de cada una en relación con la variable objetivo. Esto
permite enfocar los esfuerzos iniciales en un subconjunto más pequeño de
características, asegurando que el tiempo invertido sea productivo.

Una métrica particularmente útil para este propósito es la "Información Mutua"
(MI, por sus siglas en inglés). A diferencia de la correlación, que solo detecta
relaciones lineales, la Información Mutua puede capturar cualquier tipo de
relación entre las variables. Es fácil de usar, eficiente desde el punto de
vista computacional, bien fundamentada teóricamente y resistente al sobreajuste.
La MI mide cómo el conocimiento de una característica reduce la incertidumbre
sobre la variable objetivo, lo que la convierte en una herramienta valiosa,
especialmente en las etapas iniciales del desarrollo de características.

It's possible for a feature to be very informative when interacting with other
features, but not so informative all alone. MI can't detect interactions
between features. It is a univariate metric.
...
# Convertir una columna numérica en categórica y otra en entera
X['sepal length (cm)'] = pd.cut(X['sepal length (cm)'], bins=3, labels=["low", "medium", "high"])
X['sepal width (cm)'] = X['sepal width (cm)'].astype(int)

# Calcular la información mutua
# Aquí, establecemos discrete_features para manejar variables mixtas
mi = mutual_info_classif(X, y, discrete_features=[True, True, False, False])
mi_df = pd.DataFrame({'attributes': X.columns, 'importance': mi})
print(mi_df)
...
X = X.apply(lambda x: pd.cut(x, bins=3, labels=["low", "medium", "high"]))
...
data(iris)
# Convertir las columnas numéricas en categóricas
iris[, 1:4] = lapply(iris[, 1:4], function(x) cut(x, breaks = 3, labels = c("low", "medium", "high")))

# Calcular la información mutua
result = information_gain(Species ~ ., data = iris)
print(result)
...
print("Información Mutua con sklearn:")
print(mi)
...
# Calcular la información mutua
result = information.gain(Species~., iris)

# Mostrar los resultados
print("Información Mutua con FSelector:")
print(result)
```

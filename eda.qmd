...
::: {.callout-tip}
### Preprocesamiento de datos de perfil
...
bx(.df = ctl_01, .x = diag, .y = thoughput_dl)
```
...
```{r}
# Generar mosaicos
lista_mosaicos <- map2(.x = comb[, 1],
                       .y = comb[, 2], ~ graficar_mosaicos(df = ctl_fc,
                                                           f1 = .x,
                                                           f2 = .y,
                                                           ra = 0.3,
                                                           leyenda = TRUE)) |>
...
ctl_n |> 
 drop_na() |> 
 plot_correlation(type = "c", ggtheme = drako)
```
